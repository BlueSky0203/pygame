...
def draw_lives(surf, lives, img, x, y):
    for i in range(lives):
        ...
        img_rect.x = x + 30 * i # draw player 30px another one
        img_rect.y = y
        surf.blit(img, img_rect)

def draw_init():
    screen.blit(background_img, (0, 0))
    draw_text(screen, 'Welcome to Space Shooter!', 32, WIDTH/2, HEIGHT/4)
    draw_text(screen, '← → move space ship, keyboard (space) shoot the bullet', 14, WIDTH/2, HEIGHT/1.8)
    draw_text(screen, 'enter any keyboard to start the game~', 14, WIDTH/2, HEIGHT/2)
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # change the player background from black to transparent
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
        self.radius = 20
        # add the new property circle to circle_collide function
    ...
    def update(self):
        ...
        # Do not let player over the screen
        if self.rect.right > WIDTH:
            self.rect.right = WIDTH
        if self.rect.left < 0:
            self.rect.left = 0
    def shoot(self):
        # if player die, player can't shoot the bullet
        if not(self.hidden):
            if self.gun == 1:
...
class Rock(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.rect.x += self.speedx
        # if rock over bottom, left, right, rock falling again
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, center, size):
        pygame.sprite.Sprite.__init__(self)
        self.size = size
        self.image = expl_anim[self.size][0] # explosion first picture
        self.rect = self.image.get_rect()
        self.rect.center = center
        self.frame = 0
        # picture update
...
class Power(pygame.sprite.Sprite):
    ...
    def update(self):
        self.rect.y += self.speedy
        # if reward over the screen then delete
...
while running:
    ...
    # update game
    all_sprites.update()
    # if bullets collide with rocks that they disappear
    hits = pygame.sprite.groupcollide(rocks, bullets, True, True)
    for hit in hits:
        random.choice(expl_sounds).play()
        # according to the rock size to add your score
        score += hit.radius
        expl = Explosion(hit.rect.center, 'large') # expl = explosion
        all_sprites.add(expl)
        # broke the rocks have 10% to drop power
    ...
    # judge rocks and bullets collide
    hits = pygame.sprite.spritecollide(player, rocks, True, pygame.sprite.collide_circle)
    for hit in hits:
        new_rock()
        # Use the collide_circle is more accuracy
        ...
        if player.health <= 0:
            ...
            die_sound.play()
            # when player death_expl -1 live and recover 100 HP
            player.lives -= 1
            player.health = 100
            player.hide()

    # judge powers and bullets collide
